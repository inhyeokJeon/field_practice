...
class LandPriceAttribute(Thread):
    def __init__(self,fro,to):
        ...
        self.sql = "INSERT INTO LandPriceAttribute (pnu,ldCode,ldCodeNm,regstrSeCode,\
                regstrSeCodeNm,mnnmSlno,stdLandSn,stdrYear,bsnsDstrcAr,lndcgrCode,lndcgrCodeNm,\
    ...
    def run(self):
        temp =[]
        for row in self.get_db_data('SELECT ldongCd FROM bubjungdong where ldongCd >= %s and ldongCd < %s'):
            print(row)
            for i in range(1,3): # 
                ...
                for item in self.get_api_data():
                    var = (item['pnu'], item['ldCode'], item['ldCodeNm'], item['regstrSeCode'],
                        ...
                        item['bsnsDstrcAr'], item['lndcgrCode'], item['lndcgrCodeNm'], item['lndpclAr'], 
                        ...
                        item['lastUpdtDt'])
                    temp.append(var)
        self.set_db_data(self.sql, temp)
            
def main():
    START = 1100000000
    MID = 3056516013
    END = 5013032026
    test1 = LandPriceAttribute(START,MID)
    test2 = LandPriceAttribute(MID,END+1)
