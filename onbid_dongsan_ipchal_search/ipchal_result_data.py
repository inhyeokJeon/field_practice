...
class ipchal_result_data():
    ...
    def get_detail_data(self):
        ...
        self.dict_ipchal_result_info['exist'] = True
        if "압류" in self.dict_ipchal_result_info["재산구분"]:
            self.dict_ipchal_result_info['exist'] = False
    ...
    def run(self):
        ...
        if self.dict_ipchal_result_info['exist']:
            self.open_ipchal_history()
            self.get_ipchal_number()
            driver.back()

        print(json.dumps(self.dict_ipchal_result_info, indent=2, ensure_ascii=False))
